...
def filter_vt_data(vt_file, sus_file):
    ...
    def categorize(indicators, indicator_type):
        ...
        for item, stats in indicators.items():
            if stats.get(
                "error"
            ):  # Didn't get VirusTotal check, manual investigation needed.
...
def generate_report(sus_data, report_file):
    ...
    if total_threats == 0:
        print(
            "[!] No malicious or suspicious indicators found. Generating empty report."
        )
    ...
    if example_path.exists():
        print(
            "[*] Found 'example_report.md'. Including layout example in the prompt..."
        )
    ...
    if example_text:
        user_prompt += (
            f"### EXPECTED OUTPUT LAYOUT AND STYLE EXAMPLE:\n{example_text}\n\n---\n\n"
        )
    ...
    try:
        ...
        print(
            "[-] Please ensure LM Studio is open, a model is loaded, and the Local Server is running on port 1234."
        )
