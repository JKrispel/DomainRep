...
def is_external_domain(domain_str, internal_domain=None):
    ...
    return not (internal_domain and domain_str.endswith(internal_domain))
...
def parse_pcap_to_json(pcap_path, output_path, internal_domain=None):
    ...
    cap = pyshark.FileCapture(
        pcap_path, display_filter="dns or http or tls.handshake.type == 1"
    )
    ...
    for pkt in cap:
        ...
        if hasattr(pkt, "ip"):
            ...
            if is_public_ip(pkt.ip.src):
                data["public_ips"].add(pkt.ip.src)
