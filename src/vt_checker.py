...
def load_cache():
    ...
    if os.path.exists(CACHE_FILE):
        with open(CACHE_FILE, "r", encoding="utf-8") as f:
            print("[+] Cached VirustTotal data loaded!")
            ...
            for category in ["domains", "ips"]:
                for item, content in raw_data.get(category, {}).items():
                    if isinstance(content, dict) and "cached_at" in content:  # noqa: SIM102
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"  [-] Fatal API Error: HTTP {response.status_code} (Check your API Key)"
                )
                ...
                print(
                    f"  [-] VT Server Error (HTTP {response.status_code}). Waiting 30 seconds..."
                )
                time.sleep(30)
                continue
...
def run_vt_check(api_key, input_file, output_file):
    ...
    print(
        f"[*] Remaining API checks: {len(domains_to_check)} domains and {len(ips_to_check)} IPs."
    )
    items_left = len(domains_to_check) + len(ips_to_check)

    for domain in domains_to_check:
        print(
            f"[>] Checking domain: {domain}... ({items_left} left, estimated time: {datetime.timedelta(seconds=items_left * 16)})"
        )
        stats = check_vt(domain, api_key, "domains")

        if stats:
            cache["domains"][domain] = {"stats": stats, "cached_at": time.time()}
            save_cache(cache)
    ...
    for ip in ips_to_check:
        print(
            f"[>] Checking IP: {ip}... ({items_left} left, estimated time: {datetime.timedelta(seconds=items_left * 16)})"
        )
        stats = check_vt(ip, api_key, "ip_addresses")

        if stats:
            cache["ips"][ip] = {"stats": stats, "cached_at": time.time()}
            save_cache(cache)
